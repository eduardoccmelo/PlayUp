...
type SavedProfile = {
  user: CurrentUser;
  groups: PlayerGroup[];
  /** Composite `groupId:gameId` keys; game ids alone are only unique within a group. */
  guestGameIds: string[];
};
const gameKey = (groupId: string, gameId: number) => `${groupId}:${gameId}`;
...
const GuestTag = ({ player, language }: { player: Player; language: "pt" | "en" }) =>
  player.isGuest ? (
...
export function PlayUpApp() {
  ...
  useEffect(() => {
    ...
    const legacyGames = getStoredValue<GameSession[]>("playup.sessions.v2", []);
    const legacyRequests = getStoredValue<ParticipationRequest[]>(
      "playup.requests.v3",
      [],
    );
  ...
  useEffect(() => {
    if (
      !currentUser ||
      (!currentUser.devGodMode && currentUser.demoSeedVersion === SEED_VERSION)
    )
      return;
        ...
        if (group.id === PARTICIPANT_SEED_GROUP_ID) {
          ...
          return {
            ...
            games: [
              ...demoGroup.games,
              participantOrganizerDemoGame(
                Math.max(...demoGroup.games.map((gameSession) => gameSession.id)) + 1,
                currentUser.id,
                ownedPlayer.id,
              ),
            ],
      ...
      groups
        .flatMap((group) =>
          group.games.filter((gameSession) => {
            if (guestGameIds.includes(gameKey(group.id, gameSession.id)))
              return true;
  ...
  useEffect(() => {
    ...
    if (
      gamesWithValidTeams.some(
        (gameSession, index) => gameSession !== games[index],
      )
    ) {
  ...
  const createOrEdit = (e: FormEvent) => {
    ...
    if (
      ...
      setGameFormError("Preencha todos os detalhes do jogo.");
      return;
    }
    if (!isValidIsoDate(gameForm.date)) {
      setGameFormError("Informe uma data válida no formato DD/MM/AAAA.");
      return;
    }
    ...
    if (
      ...
      setGameFormError("Já existe um jogo com esses mesmos dados.");
      return;
    }
    if (editGame) {
  ...
  const leaveMyGame = (selectedGame: GameSession) => {
    const selectedGroup = groups.find((group) =>
      group.games.some((gameSession) => gameSession === selectedGame),
    );
    ...
    setGuestGameIds((current) =>
      current.filter(
        (key) => key !== gameKey(selectedGroup.id, selectedGame.id),
      ),
    );
  };
  ...
  const markRequestRead = (request: ParticipationRequest) =>
      ...
      !user
        ...
        : {
            ...
            adminGroupIds: (user.adminGroupIds ?? []).filter(
              (id) => id !== groupToLeave.id,
            ),
            leftGroupIds: [
              ...new Set([...(user.leftGroupIds ?? []), groupToLeave.id]),
            ],
          },
    );
    setGuestGameIds((keys) =>
      keys.filter((key) => !key.startsWith(`${groupToLeave.id}:`)),
    );
    return true;
  };
  const saveProfile = (
    ...
    const restoreSavedProfile = (profile: SavedProfile) => {
      const seedIds = new Set(seedGroups().map((group) => group.id));
      const refreshedSeedGroups = groups.filter((group) => seedIds.has(group.id));
      const savedCustomGroups = profile.groups.filter(
        (group) => !seedIds.has(group.id),
      );
      setGroups([...refreshedSeedGroups, ...savedCustomGroups]);
      // Profiles saved before composite keys stored bare numeric ids; drop those.
      setGuestGameIds(
        ((profile.guestGameIds ?? []) as unknown[]).filter(
    ...
    if (mode === "sign-in") {
      ...
      restoreSavedProfile(savedProfile);
      return null;
    }
  ...
  const withProfile = (page: ReactNode) => (
    <>
      {page}
      <LandingPage
        key={`${currentUser?.id ?? "anonymous"}:${currentUser?.email ?? ""}`}
        profileOnly
        language={language}
        user={currentUser}
        openProfile={profileRequested}
        onCloseProfile={() => setProfileRequested(false)}
    ...
    return withProfile(
      <GroupSelector
          ...
          setCurrentUser((user) =>
            user
              ? {
                  ...user,
                  adminGroupIds: [
                    ...new Set([...(user.adminGroupIds ?? []), group.id]),
                  ],
                }
              : user,
          );
        ...
        onChoose={(group) => {
          ...
          setEntered(false);
          setPick("");
          setAccess(adminGroupIds.includes(group.id) ? "admin" : "player");
        }}
        ...
        onJoinGroup={(code, passcode) => {
          ...
          if (!currentUser?.emailVerified) {
            return "Confirme seu nome e e-mail para entrar neste grupo.";
          }
        ...
        onAddMyGame={(code) => {
          ...
          if (!currentUser?.emailVerified) {
            return "Confirme seu nome e e-mail para entrar neste jogo.";
          }
          setGroups((currentGroups) =>
            currentGroups.map((group) => {
              if (group.id !== match.group.id) return group;
              const existingPlayer = group.players.find(
                (player) => player.ownerUserId === currentUser.id,
              );
              const player: Player = existingPlayer ?? {
                id: nextIdentifier(group.players),
              ...
              return {
                ...group,
                players: existingPlayer ? group.players : [...group.players, player],
                games: group.games.map((gameSession) => {
                  if (gameSession.id !== match.gameSession.id) return gameSession;
                  if (
                    gameSession.playerIds.includes(player.id) ||
                    gameSession.waitlistIds.includes(player.id)
                  )
                    return gameSession;
                  return gameSession.playerIds.length < gameSession.maxPlayers
                    ? { ...gameSession, playerIds: [...gameSession.playerIds, player.id] }
                    : { ...gameSession, waitlistIds: [...gameSession.waitlistIds, player.id] };
                }),
              };
            }),
          );
          const key = gameKey(match.group.id, match.gameSession.id);
          setGuestGameIds((current) =>
            current.includes(key) ? current : [...current, key],
          );
          setActiveGroupId(match.group.id);
          setGameId(match.gameSession.id);
          setReturnToDashboardAfterGame(true);
          setAccess("player");
          return null;
        }}
  ...
  if (access === "player")
    return withProfile(
      <PlayerView
        ...
        onLeaveGroup={() => {
          if (!activeGroup || !leaveGroup(activeGroup)) return false;
          setActiveGroupId(null);
          setAccess("groups");
          return true;
        }}
    ...
    return withProfile(
      <main className="app-shell">
        ...
        <section className="game-directory-heading game-management-heading">
          {activeGroup?.name && (
            <p className="page-group-name">{activeGroup.name}</p>
          )}
        ...
        <section className="panel session-panel game-management-list">
          {games.filter(hasGameEnded).length > 0 ? (
            <div className="session-tabs past-games-list">
              {[...games]
                .filter(hasGameEnded)
                .sort(chronological)
                .map((gameSession, index, pastGames) => (
                  <Fragment key={gameSession.id}>
                  <div className="session-row ended">
                    <CompactGameDetails
                      game={gameSession}
                      language={language}
                    />
  ...
  const editGameDetails = (gameSession: GameSession, keepGameOpen = false) => {
  ...
  return withProfile(
    <main className="app-shell">
        ...
        <section className="hero game-hero">
          ...
          {!hasGameEnded(game) && (
            <div className="hero-game-actions">
              <button
                className="primary hero-game-action"
                onClick={() => {
          ...
          {games.some((gameSession) => !hasGameEnded(gameSession)) ? (
            <div className="session-tabs">
              {[...games]
                ...
                .sort(chronological)
                .map((gameSession, index, futureGames) => {
                  return (
                    <Fragment key={gameSession.id}>
                    <div
          ...
          >
            ...
            {gameFormError && (
              <small className="error">
                {localize(gameFormError, language)}
              </small>
            )}
            <GameForm
              draft={gameForm}
                    ...
                    {playerSearchResults.map((player) => {
                      ...
                      return (
                        <div key={player.id}>
                          <span>
                            {player.name}
                            <GuestTag player={player} language={language} />
                            {isCurrentUserPlayer(player, currentUser) &&
                  ...
                  return (
                    ...
                    >
                      <div className="player-name">
                        <strong>
                          {p.name}
                          <GuestTag player={p} language={language} />
                          <span
                            aria-label={localize(
                      ...
                      <div className="player-row-actions">
                        {game.playerIds.includes(p.id) ? (
                          ...
                          >
                            {localize("Incluir na lista", language)}
                          </button>
                        ) : (
                          <button
                            className="text-button"
                    ...
                    return (
                      <div className="waitlist-player" key={waitingPlayer.id}>
                        <strong>
                          ...
                          {waitingPlayer.name}
                          <GuestTag player={waitingPlayer} language={language} />
                        </strong>
                        <button
                          aria-label={localize(
                            "O jogador será incluído na última vaga pendente.",
                      ...
                      >
                        ...
                        {[1, 2, 3, 4, 5, 6, 12, 24].map((hours) => (
                          <option key={hours} value={hours}>
                            {hours}{" "}
                            {localize(hours === 1 ? "hora" : "horas", language)}{" "}
            ...
            <aside className="side-column">
              <section className="teams-section team-balance-panel">
                ...
                {!isParticipantGameManager ? (
                      ...
                      ? latestBalance.type === "late-rebalance"
                        ...
                        : latestBalance.type === "admin"
                          ? language === "pt"
                            ...
                            : `Balance triggered by ${latestBalance.adminName}. Balance: ${latestBalance.count} of ${currentBalanceLimit}.`
                          : language === "pt"
                            ? `Balanceamento automático acionado pelo sistema. Balanceamento: ${latestBalance.count} de ${currentBalanceLimit}.`
                            : `Balance triggered automatically by the system. Balance: ${latestBalance.count} of ${currentBalanceLimit}.`
                  ...
                  <p className="balance-availability">
                    {localize("Balanceamentos", language)}:{" "}
                    {balanceCountOf(game)} {localize("de", language)}{" "}
                    {currentBalanceLimit}
                  </p>
                ) : null}
                {balanceCountOf(game) >= currentBalanceLimit &&
                    ...
                    updateGame(
                      {
                        ...
                        lateRebalanceUsed:
                          isLateRebalance || game.lateRebalanceUsed,
                      },
                      false,
                    );
                  }}
      ...
      {isGameInviteOpen && game && activeGroup && (
        <div className="modal-backdrop" role="presentation">
          <section
            aria-modal="true"
          ...
          >
            <p className="form-mode">
              {localize("CONVITE PARA O JOGO", language)}
            </p>
            <p>
              {localize(
            ...
            <input readOnly value={gameInviteCode(activeGroup, game)} />
...
function PlayerView({
  ...
  onManageGroup?: () => void;
  /** Returns false when leaving is not allowed (an owner without a successor). */
  ...
  const [isGroupLeaveConfirmationOpen, setIsGroupLeaveConfirmationOpen] =
    useState(false);
  const [groupLeaveError, setGroupLeaveError] = useState("");
  ...
  const paidPlayersCount =
    game?.playerIds.filter((playerId) => game.paidPlayerIds.includes(playerId))
      .length ?? 0;
  const list = game && (
                ...
                >
                  <span className="readonly-player-name">
                    <strong>
                      ...
                      {(() => {
                        const player = players.find((p) => p.id === x);
                        return player ? <GuestTag player={player} language={language} /> : null;
                      })()}
                      {isCurrentUserPlayer(
                        players.find((p) => p.id === x)!,
                        user,
                      ) && ` (${localize("você", language)})`}
                    </strong>
                  </span>
                      ...
                      <>
                        <label className="readonly-payment-confirmation">
                          <input
                            ...
                            onChange={(event) =>
                              confirmPayment(x, event.target.checked)
                            }
                            type="checkbox"
                          />
                          <span>{localize("Pago", language)}</span>
  ...
  return (
    <main className="participant-page">
      ...
      {!isViewingGame ? (
        <>
          <section
            className={`game-directory-heading participant-games-heading${canCreateGame ? " participant-game-creator-heading" : ""}`}
          >
            <div>
            ...
            <div className="game-directory-actions">
              ...
              {onLeaveGroup && (
                <button
                  className="session-action-button delete participant-leave-group-button"
                  onClick={() => {
                    setGroupLeaveError("");
                    setIsGroupLeaveConfirmationOpen(true);
                  }}
                >
              ...
              <div className="session-tabs participant-game-list">
                {games.map((g, index) => {
                  ...
                  const isCurrentUserOrganizer =
                    g.createdByRole === "participant" &&
                    g.createdByUserId === user?.id;
                  return (
                    <Fragment key={g.id}>
                    <div
                      className={`session-row ${isEnded ? "ended" : ""} ${g.cancelled ? "cancelled" : ""} ${isCurrentUserOrganizer ? "organizer-game-row" : ""}`}
                    >
                      <CompactGameDetails
                        game={g}
                        language={language}
                        waiting={isCurrentUserWaiting}
                        organizer={isCurrentUserOrganizer}
                        hideEndTime={isCurrentUserOrganizer}
                      />
                      <div className="session-row-actions">
                        <button
                          className="session-action-button view"
                        ...
                        <button
                          ...
                          disabled={
                            isEnded ||
                            g.cancelled ||
                            (!isCurrentUserAttending && !isCurrentUserWaiting)
                          }
                          onClick={() => {
                            choose(g.id);
      ...
      {isGameCreationOpen && (
        <div className="modal-backdrop" role="presentation">
          ...
          >
            ...
            {gameCreationError && (
              <small className="error">
                {localize(gameCreationError, language)}
              </small>
            )}
            <GameForm
              draft={gameDraft}
      ...
      {activeAction && game && (
        <div className="modal-backdrop" role="presentation">
          ...
          >
            <p className="form-mode">
              {localize(
                activeAction === "join" ? "Confirmar presença" : "Sair da lista",
                language,
              )}
            </p>
            <form
      ...
      {isGroupLeaveConfirmationOpen && (
        <div className="modal-backdrop" role="presentation">
          ...
          >
            ...
            {groupLeaveError && (
              <small className="error">
                {localize(groupLeaveError, language)}
              </small>
            )}
            <div className="confirm-dialog-actions">
              <button
