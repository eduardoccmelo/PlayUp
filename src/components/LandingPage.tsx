...
type LandingPageProps = {
  ...
  onSaveUser: (profile: Pick<CurrentUser, "displayName" | "email">, mode: "sign-in" | "sign-up") => string | null;
  openProfile?: boolean;
...
export function LandingPage({
  ...
  const sendAccessCode = () => {
    ...
    if (!profileChanged) {
      const error = onSaveUser({ displayName: name.trim(), email: email.trim().toLowerCase() }, mode);
    ...
    <main className="access-page landing-page">
      <div className="landing-topbar">
        <div className="landing-top-actions">
          ...
          <button aria-label="User settings" className="secondary landing-profile-button" onClick={() => setIsEditing(true)}>
      ...
      <div className="landing-content">
        <section className="landing-hero-copy">
          {user?.emailVerified ? (
            ...
            <button className="primary landing-sign-in-button" onClick={() => { setMode("sign-in"); setIsEditing(true); }}>
        ...
        <div className="access-actions">
          <button className="primary access-option" disabled={!user?.emailVerified} onClick={onEnter}>
        ...
        <div className="modal-backdrop" role="presentation">
          <section aria-modal="true" className="confirm-dialog group-auth-modal" role="dialog">
            <div className="profile-modal-heading">
              ...
              {user?.emailVerified && (
                <button className="session-action-button delete" type="button" onClick={() => { setMode("sign-in"); onLogout(); }}>
                  Logout
                </button>
              )}
            </div>
            {step === "details" ? (
              <form className="profile-form" onSubmit={(event) => { event.preventDefault(); sendAccessCode(); }}>
                {!user?.emailVerified && (
                  <div className="profile-mode-actions">
                    <button className={mode === "sign-up" ? "primary" : "secondary"} type="button" onClick={() => { setMode("sign-up"); setProfileError(""); }}>
                      {language === "pt" ? "Criar perfil" : "Create profile"}
                    </button>
                    <button className={mode === "sign-in" ? "primary" : "secondary"} type="button" onClick={() => { setMode("sign-in"); setProfileError(""); }}>
                      {language === "pt" ? "Entrar" : "Sign in"}
                    </button>
                  </div>
                )}
                {!user?.emailVerified && (
                  <p>
                    {mode === "sign-in"
                      ...
                      : localize("Crie seu perfil para recuperar seus grupos e jogos em qualquer dispositivo.", language)}
                  </p>
                )}
                {mode === "sign-up" && (
                ...
                <label className="profile-field">
                  ...
                  <input autoFocus={mode === "sign-in"} required inputMode="email" placeholder="nome@email.com" type="email" value={email} onChange={(event) => { setEmail(event.target.value); setProfileError(""); }} />
                </label>
                {!user?.emailVerified && (
                ...
                <div className={`confirm-dialog-actions${!user?.emailVerified ? " profile-entry-actions" : ""}`}>
                  <button className="secondary" type="button" onClick={closeProfile}>{localize("Cancelar", language)}</button>
                  <button className="primary">{localize(profileChanged ? "Enviar código" : "Salvar", language)}</button>
                </div>
              </form>
            ) : (
              <form className="profile-form" onSubmit={(event) => { event.preventDefault(); verifyAccessCode(); }}>
                <p>
                  {step === "verify-current-email"
                    ? language === "pt"
                      ? <>Enviamos um código de confirmação para o e-mail atual: <strong>{user?.email}</strong>.</>
                      : <>We sent a confirmation code to your current email: <strong>{user?.email}</strong>.</>
                    : step === "verify-new-email"
                      ? language === "pt"
                        ? <>Agora enviamos um código de confirmação para o novo e-mail: <strong>{email}</strong>.</>
                        : <>We have now sent a confirmation code to the new email: <strong>{email}</strong>.</>
                      : <>{localize("Enviamos um código de acesso para", language)} <strong>{email}</strong>.</>}
                </p>
                <label className="profile-field">
                ...
                {profileError && <small className="error">{localize(profileError, language)}</small>}
                <div className="confirm-dialog-actions verification-primary-actions">
                  <button className="session-action-button delete" type="button" onClick={() => { setStep("details"); setProfileError(""); setResendConfirmation(false); }}>{localize("Alterar dados", language)}</button>
                  <button className="primary" type="button" onClick={() => { setAccessCode(""); setProfileError(""); setResendConfirmation(true); }}>{localize("Enviar novo código", language)}</button>
                </div>
