...
The quota is intentionally small: balancing should be the final preparation step, preferably on game day. A real roster or payment change invalidates the displayed teams and can consume the next available automatic balance, but never resets the quota. Reordering the same player IDs is not a roster change, so it never rebalances or spends the quota. Rescheduling a game recalculates the 15-minute final-rebalance window from its new kickoff, but never resets the standard quota or grants another final rebalance. If an admin brings the game forward, the early-availability notice identifies that admin; if the final rebalance is used, the same attribution is retained in its history entry.
