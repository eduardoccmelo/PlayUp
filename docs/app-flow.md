...
6. Balanceamento usa apenas jogadores confirmados e pagos. Jogo de admin tem duas gerações regulares; jogo criado por participante tem uma. Depois existe apenas um rebalanceamento manual final, liberado nos últimos 15 minutos antes do horário vigente do jogo. Alterar lista ou pagamento não reinicia limites. Ao remarcar data ou horário, a janela final é recalculada para os 15 minutos anteriores ao novo início, mas não concede outro rebalanceamento final nem reinicia a cota. A remarcação registra horário anterior, novo horário e admin responsável. Se ela antecipar o início, o aviso da janela final e, caso usado, o histórico do rebalanceamento final mostram essa autoria. Uma substituição só preserva os times se o conjunto de assinaturas de balanceamento continuar igual: posição, nível, velocidade e condição; IDs e ordem da lista são ignorados. Caso contrário, dispara o próximo balanceamento automático disponível.
